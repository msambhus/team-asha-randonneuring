...
# ── Promoted plan-view / strategy helpers (re-export shims) ──────────────────
# These pure display/strategy/risk functions were promoted verbatim into the
# flask-free shared/ package so BrevetHub can reuse the SAME implementation (see
# shared/plan_view.py + shared/strategies.py). They are re-exported here under
# their original names so every existing Team Asha caller — and the tests that
# `from routes.riders import ...` them — resolve byte-identically. compute_risk_zones
# gained optional lat/lon params (defaulted None) for multi-club sunrise; the Team
# Asha call site passes none, so its behavior is unchanged.
from shared.plan_view import (  # noqa: F401  (re-exported for callers/tests)
    _HEADWIND_FT_PER_MPH, _TAILWIND_FT_PER_MPH, _temp_penalty,
...
# Server-side cap on a stored rider note (also bounds the coach prompt).
...
def save_ride_notes(rusa_id, ride_id):
    ...
    # Owner guard: only the profile owner may write notes (same check the page
    # uses to decide whether to show the editable fields).
    ...
    note = (payload.get('note') or '').strip()[:MAX_RIDE_NOTE_LEN]
    ...
    if request.is_json:
        return jsonify({'ok': True, 'scope': scope, 'note': note})
    return redirect(url_for('riders.ride_strava_analysis', rusa_id=rusa_id, ride_id=ride_id))
