"""Garmin LiveTrack ingestion engine — canonical, club- and framework-agnostic.
...
Like ``shared/rwgps.py`` this is the SINGLE implementation: it imports only the
stdlib + ``requests`` and touches no web-framework application globals, so BOTH apps in
this monorepo can reuse it. ``services/garmin_livetrack.py`` is a pure re-export
shim of this module (see ``tests/test_garmin_livetrack_shim.py``), and
``brevethub/shared/garmin_livetrack.py`` is a byte-identical vendored copy (see
``tests/brevethub/test_vendored_shared_sync.py``), so Team Asha and BrevetHub
...
Design notes:
  ...
  - All network access uses `requests` with a timeout and explicit handling of
    404 / 401 / 429 / 5xx, mirroring shared/rwgps.py.
  ...
  - Callers treat this as fail-soft: on any error we raise a descriptive
    ...
    breaks the batch.
  - Logging uses the stdlib ``logging`` module (no web-framework request-context
    logger) so the module stays importable outside any web request context — the
    ``shared/`` isolation contract, enforced by test_shared_isolation.py.
"""
import json
import logging
import re
